...
bool MyWebserver::connectToWifi(const char* ssid, const char* pass){
...
bool MyWebserver::begin(){
  ...
  //prepare the site for the first time with the local_ip
  prepareSite();
...
void MyWebserver::handleSubmitPath() {
  ...
  //could do some validity checking here
  //Serial.println(server->arg(0));
  
  //if (server->args() == 0 ) return;
  if (server->argName(0) != "adresses") return;
  String elementsString = server->arg(0); //"firstElement&secondElement";
